...
sdk = mercadopago.SDK(
    "APP_USR-8369192826034467-062805-d94db51d5c00a042fa3f2c2a3b952c2b-1481851807"
)
...
def criar_pagamento():
    ...
    preference_data = {
        "items": [
            {"title": descricao, "quantity": 1, "unit_price": float(valor_total)}
        ],
        "back_urls": {
    ...
    try:
        ...
        print(
            "Resposta do MercadoPago:", payment_response
        )  # <-- Debug para ver a resposta completa
        ...
        return (
            jsonify({"error": "Erro ao processar pagamento.", "details": str(e)}),
            500,
        )
    finally:
        if "cursor" in locals():
